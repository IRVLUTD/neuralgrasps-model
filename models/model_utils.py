...
def get_model(model_directory, specs, device):
    ...
    if model_type == "PC_2encoder1decoder_VAE":
        ...
        encoder_obj = arch.ResnetPointnet(
            c_dim=half_latent_size, hidden_dim=256)
        # hand encoder get 2xlatent_size, half for mean, another for variance.
        encoder_hand = arch.ResnetPointnet(
            c_dim=latent_size, hidden_dim=256, cond_dim=latent_size)

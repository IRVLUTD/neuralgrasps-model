...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    num_grippers = get_spec_with_default(
        specs, "NumGrippers", 3)

    # number of sdf outputs will be num_grippers + 1 (for object)
    decoder = arch.MultiSDFDecoder(
        grp_embedding_size + latent_size, num_grippers+1, **specs["NetworkSpecs"]).cuda()
        ...
        for idx_dataloader, (grp_idxs, sdf_data, indices, _) in enumerate(sdf_loader):
            ...
            for i in range(batch_split):                
                ...
                if grp_embedding_size > 0:
                    ...
                    input_dsdf = torch.cat(
                        [batch_vecs, xyz[i]], dim=1).cuda()

                # NN optimization
                predictions = decoder(input_dsdf)
                # Last index corresponds to the object sdf
                pred_sdf_obj = predictions[:, num_grippers].unsqueeze(1)
                pred_sdf_grp = torch.gather(predictions, 1, grp_idxs[i].unsqueeze(1).cuda())
